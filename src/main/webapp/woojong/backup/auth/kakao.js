  ...
  var define, module, exports;return function e(t, n, r) {
    function s(o, u) {
      ...
      module.exports = function () {
        ...
        Link.createScrapkWithTemplate = Link.createScrapButtonWithTemplate = function (settings) {
          settings = _k.processRules(settings, rules.createScrapkWithTemplate, 'Link.createScrapkWithTemplate');

          var container = _.getElement(settings.container);
          if (!container) {
            throw new _k.KakaoError('container is required for KakaoTalk Link(scrap): pass in element or id');
          }

          var handler = function handler() {
            buildScrapWithTemplate(settings, 'Link.createScrapkWithTemplate');
          };
        ...
        Link.sendScrapWithTemplate = function (settings) {
          var settings = _k.processRules(settings, rules.scrapWithTemplate, 'Link.sendScrapWithTemplate');
          return buildScrapWithTemplate(settings, 'Link.sendScrapWithTemplate');
        };
        ...
        rules.createScrapkWithTemplate = _.defaults({
          required: { requestUrl: _.isString, container: _.passesOneOf([_.isElement, _.isString]) }
        }, rules.scrapWithTemplate);
        ...
        function buildUrlWithTemplate(settings) {
          return Kakao.API.request({
            ...
            var linkWithTemplateResponse = {
              'template_msg': res.template_msg || {},
              'warning_msg': res.warning_msg || {},
              'argument_msg': res.argument_msg || {}
            };
            settings.always(linkWithTemplateResponse);
            settings.success(linkWithTemplateResponse);
            ...
            return linkWithTemplateResponse;
          }, function (e) {
        ...
        function buildScrapWithTemplate(settings) {
          var requestData = {
            'link_ver': '4.0',
          ...
          return Kakao.API.request({
            ...
            var scrapWithTemplateResponse = {
              'template_id': res.template_id || -1,
              'template_args': res.template_args || {},
              'template_msg': res.template_msg || {},
              'warning_msg': res.warning_msg || {},
              'argument_msg': res.argument_msg || {}
            };
            settings.always(scrapWithTemplateResponse);
            settings.success(scrapWithTemplateResponse);
            var linkWithTemplate = new KakaoTalkLinkWithTemplate();
            linkWithTemplate.extras = _.extend(linkWithTemplate.extras, settings.extras);
            linkWithTemplate.template_json = res.template_msg;
            linkWithTemplate.template_args = res.template_args;
            linkWithTemplate.template_id = res.template_id;
            var url = linkScheme + '://send?' + _.buildQueryString(linkWithTemplate);
            if (url.length > 10000) {
              throw new _k.KakaoError('Failed to send message because it exceeds the message size limit. Please contact the app administrator.');
            }
            sendLink(url, settings.fail, settings.installTalk);
            return scrapWithTemplateResponse;
          }, function (e) {
            _logger("talklink_scrap_4_0_error", e.message || e);
            throw e;
          });
        }
